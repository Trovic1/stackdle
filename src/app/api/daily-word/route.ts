...
import { getTodaysWord, getGameId, ANSWERS } from '../../lib/words';
import { createHash } from 'crypto';

/**
 * Server-side comprehensive word list for validation.
 * This is a large set of common 5-letter English words
 * that covers the vast majority of words players will try.
 * Loaded once at server startup.
 */
const SERVER_VALID_WORDS: Set<string> = new Set([
  // All answer words are valid
  ...ANSWERS,
...
async function getValidWords(): Promise<Set<string>> {
  ...
  try {
    ...
    cachedWords = new Set([...ANSWERS, ...SERVER_VALID_WORDS, ...words]);
    return cachedWords;
  } catch (error) {
    console.error('Failed to load full word list, falling back to local list:', error);
    return SERVER_VALID_WORDS;
  }
}
...
export async function GET() {
  ...
  // Return a SHA-256 hash so the client can't see the answer
  const wordHash = createHash('sha256').update(todaysWord.toLowerCase()).digest('hex');
...
/**
 * POST endpoint to evaluate a guess.
 * Implements the official Wordle two-pass evaluation algorithm.
 */
export async function POST(request: Request) {
  try {
    const { guess, isFinalGuess } = await request.json();
    ...
    for (let i = 0; i < 5; i++) {
      if (guessChars[i] === answer[i]) {
        evals[i] = 'correct';
        remainingAnswer[i] = null; // Remove from pool
      }
    }

    // PASS 2: Mark YELLOW (present but wrong position) or GRAY (absent)
    for (let i = 0; i < 5; i++) {
      if (evals[i] === 'correct') continue; // Already GREEN
      
      const idx = remainingAnswer.indexOf(guessChars[i]);
      if (idx !== -1) {
        evals[i] = 'present';
        remainingAnswer[idx] = null; // Remove ONE occurrence
      }
      // else stays 'absent' (GRAY)
    }
